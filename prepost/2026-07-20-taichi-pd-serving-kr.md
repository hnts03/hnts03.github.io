...
- **단일 노드 평가**: A100 8장 단일 노드에서만 검증했다. 다중 노드 성능은 확인되지 않았다. 즉, internode kv xfer 에 대한 요소가 미반영.
- **오프라인 설정 탐색**: 슬라이더 최적값을 오프라인으로 찾아야 하고, 워크로드가 크게 바뀔 때만 재설정한다. 실시간 적응은 아니다. -- 이러한 휴리스틱을 auto-tuning 하는것이 새로운 연구로 이어질 수 있겠다.
- **반응형 스케줄링**: 출력 길이를 미리 알 수 없어, 예측이 아니라 사후 반응으로 요청을 이주시킨다. 이와 비슷한 접근으로 windServe 라는 논문이 있다. 
...
한계점은 명확하다. 단일 노드 검증과 오프라인 슬라이더 탐색은 대규모 클러스터의 동적 워크로드에 그대로 적용하기 어렵다. 다중 노드로의 확장과 온라인 적응이 후속 과제로 남는다. 그럼에도 PD 논쟁을 통합의 문제로 바꾼 관점은 이후 서빙 시스템 설계에 참고할 가치가 있다고 생각한다.
