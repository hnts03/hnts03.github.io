...
GA100(A100)은 데이터센터 전용으로 RT Core가 없다. GA102(RTX 30xx)는 **2세대 RT Core**를 탑재했다. Turing 1세대 대비 BVH 처리량이 향상됐다. GA100의 최대 occupancy는 GV100과 동일하게 64 warps/SM이며, GA102는 48 warps/SM으로 중간값이다.
...
Ampere는 **PCIe 4.0 x16**을 NVIDIA GPU 최초로 채택했다. 이론 단방향 대역폭은 32 GB/s(PCIe 3.0의 2배)다.
...
GA102(RTX 30xx)는 **AV1 하드웨어 인코드를 최초로 지원**하는 세대다. A100은 데이터센터 전용으로 NVENC, NVDEC, 디스플레이가 없다.
