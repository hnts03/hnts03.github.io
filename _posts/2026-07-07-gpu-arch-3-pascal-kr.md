...
GP100 SM은 최대 **64 warps/SM, 32 블록/SM**을 동시에 보유할 수 있다.
...
GP102/GP104 소비자 GPU는 **PCIe 3.0 x16**(단방향 16 GB/s)으로 호스트에 연결된다. GP100(P100)도 PCIe 3.0 x16 인터페이스를 갖추며, GPU 간 통신에는 NVLink 1.0을 별도로 사용한다.
...
| | GP100 (P100) | GP102 / GP104 (소비자) |
|:---|:---:|:---:|
| NVENC | 없음 | **3세대** (H.264, HEVC 10-bit) |
| NVDEC | 없음 | **3세대** (HEVC 10-bit 디코드) |
| 디스플레이 출력 | 없음 | 있음 |

GP100은 데이터센터 전용으로 NVENC, NVDEC, 디스플레이 출력이 없다.
